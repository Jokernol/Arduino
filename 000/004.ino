/*
    ...
    sketch：21206byte、local variable:217byte free
...
#define MIN_TRIG_SWING \
    5  // minimum trigger swing.(Display "Unsync" if swing smaller than this
       // value
...
const char vRangeName[10][5] PROGMEM = {
    "A50V", "A 5V", " 50V", " 20V", " 10V", "  5V",
    "  2V", "  1V", "0.5V", "0.2V"};  // Vertical display character (number of
                                      // characters including \ 0 is required)
const char* const vstring_table[] PROGMEM = {
    vRangeName[0], vRangeName[1], vRangeName[2], vRangeName[3], vRangeName[4],
    vRangeName[5], vRangeName[6], vRangeName[7], vRangeName[8], vRangeName[9]};
const char hRangeName[10][6] PROGMEM = {
    "200ms", "100ms", " 50ms", " 20ms", " 10ms", "  5ms",
    "  2ms", "  1ms", "500us", "200us"};  //  Hrizontal display characters
...
const PROGMEM float hRangeValue[] = {
    0.2,   0.1,   0.05,   0.02,  0.01, 0.005,
    0.002, 0.001, 0.5e-3, 0.2e-3};  // horizontal range value in second. ( =
                                    // 25pix on screen)
...
float lsb5V = 0.00566826;   // sensivity coefficient of 5V range.
...
volatile int vRange;  // V-range number 0:A50V,  1:A 5V,  2:50V,  3:20V,  4:10V,
                      // 5:5V,  6:2V,  7:1V,  8:0.5V,  9:0.2V
...
void setup() {
    ...
    // oled.begin(SSD1306_SWITCHCAPVCC, 0x3C) { // select 3C or 3D (set your
    // OLED I2C address)
...
void loop() {
    ...
    setConditions();  // set measurment conditions again (reflect change during
                      // measure)
...
void writeCommonImage() {                   // Common screen image drawing
    ...
    oled.drawFastHLine(123, 9, 5,
                       WHITE);  // right side Max value auxiliary mark
    oled.drawFastHLine(123, 63, 5, WHITE);

    for (int x = 26; x <= 128; x += 5) {
        oled.drawFastHLine(x, 36, 2,
                           WHITE);  // Draw the center line (horizontal line)
                                    // with a dotted line
    }
    for (int x = (127 - 25); x > 30; x -= 25) {
        for (int y = 10; y < 63; y += 5) {
            oled.drawFastVLine(
                x, y, 2, WHITE);  // Draw 3 vertical lines with dotted lines
        }
    }
}
...
void readWave() {               // Record waveform to memory array
    ...
    switch (hRange) {  // set recording conditions in accordance with the range
                       // number
        case 0: {      // 200ms range
        ...
        case 6: {                    // 2ms range
            ...
            ADCSRA = ADCSRA & 0xf8;  // clear bottom 3bit
            ADCSRA = ADCSRA | 0x06;  // dividing ratio = 64 (0x1=2, 0x2=4,
        ...
        case 7: {                    // 1ms range
            ...
            ADCSRA = ADCSRA & 0xf8;  // clear bottom 3bit
            ADCSRA = ADCSRA | 0x05;  // dividing ratio = 16 (0x1=2, 0x2=4,
                                     // 0x3=8, 0x4=16, 0x5=32, 0x6=64, 0x7=128)
            for (int i = 0; i < REC_LENG; i++) {  // up to rec buffer size
        ...
        case 9: {                    // 200us range
            ...
            for (int i = 0; i < REC_LENG; i++) {  // up to rec buffer size
                ...
                // time fine adjustment 0.0625 * 20 = 1.25us (nop=0.0625us
                // @16MHz)
                asm("nop");
                asm("nop");
                asm("nop");
...
void dataAnalize() {  // get various information from wave form
    ...
    // calculate average
    dataAve = (sum + 10) / 20;  // Average value calculation (calculated by 10
    ...
    if (vRange <= 1) {                    // if Autorabge(Range number <=1）
        ...
        if (att10x == 1) {  // if 10x attenuator used
            rangeMaxDisp =
                100 *
                (rangeMax *
                 lsb50V);  // display range is determined by the data.(the upper
                           // limit is up to the full scale of the ADC)
            rangeMinDisp =
                100 *
                (rangeMin * lsb50V);  // lower depend on data, but zero or more
    ...
    for (trigP = ((REC_LENG / 2) - 51); trigP < ((REC_LENG / 2) + 50);
         trigP++) {  // Find the points that straddle the median at the center ±
                     // 50 of the data range
        if (trigD == 0) {  // if trigger direction is positive
            if ((waveBuff[trigP - 1] < (dataMax + dataMin) / 2) &&
                (waveBuff[trigP] >= (dataMax + dataMin) / 2)) {
    ...
    trigSync = true;
    if (trigP >=
        ((REC_LENG / 2) + 50)) {  // If the trigger is not found in range
        trigP = (REC_LENG / 2);   // Set it to the center for the time being
        trigSync = false;         // set Unsync display flag
    }
    if ((dataMax - dataMin) <=
        MIN_TRIG_SWING) {  // amplitude of the waveform smaller than the
                           // specified value
        trigSync = false;  // set Unsync display flag
    }
    freqDuty();
...
void freqDuty() {  // detect frequency and duty cycle value from waveform data
    ...
    waveFreq = 1.0 / ((pgm_read_float(hRangeValue + hRange) * pPeriod) /
                      25.0);              // frequency
...
void dispInf() {  // Display of various information
    ...
    if (scopeP == 0) {     // if scoped
        oled.drawFastHLine(0, 7, 27,
                           WHITE);  // display scoped mark at the bottom
    ...
    voltage =
        (rangeMaxDisp + rangeMinDisp) / 200.0;  // center value calculation
    if (vRange == 1 || vRange > 4) {            // if range below 5V or Auto 5V
        dtostrf(voltage, 4, 2, chrBuff);        // format *.**
    } else {                                    // no!
        dtostrf(voltage, 4, 1, chrBuff);        // format **.*
...
void plotData() {  // plot wave form on OLED
    long y1, y2;
    for (int x = 0; x <= 98; x++) {
        y1 = map(waveBuff[x + trigP - 50], rangeMin, rangeMax, 63,
                 9);                // convert to plot address
        y1 = constrain(y1, 9, 63);  // Crush(Saturate) the protruding part
        y2 = map(waveBuff[x + trigP - 49], rangeMin, rangeMax, 63,
                 9);                                   // to address calucurate
...
void auxFunctions() {  // voltage meter function
    ...
    long x;
    if (digitalRead(8) ==
        LOW) {  // if SELECT button pushed, measure battery voltage
    ...
    if (digitalRead(9) == LOW) {  // if UP button pushed, 5V range
        ...
        pinMode(
            12,
            INPUT);  // Set the attenuator control pin to Hi-z (use as input)
        while (1) {  // do forever,
            digitalWrite(13, HIGH);           // flash LED
...
void uuPinOutputLow(unsigned int d,
                    unsigned int a) {  // 指定ピンを出力、LOWに設定
    // PORTx =0, DDRx=1
    unsigned int x;
    x = d & 0x00FF;
    PORTD &= ~x;
