...
void plotData() {  // 配列の値に基づきデーターをプロット
    ...
    long y1;
    long y2;
    for (int8_t x = 0; x <= 98; x++) {
        y1 = map(dataBuff[x + trigP - 50], rangeMin, rangeMax, 63, 9);
        y1 = constrain(y1, 9, 63);
        y2 = map(dataBuff[x + trigP - 49], rangeMin, rangeMax, 63, 9);
...
void trigVoltage() {
    long y;
    float trig;
    trig = analogRead(1) / 1023.0 * rangeMax;
    y = map(trig, rangeMin, rangeMax, 63, 9);
    y = constrain(y, 9, 63);
