...
const char vRangeName[10][5] PROGMEM = {
    ...
    "  2V", "  1V", "0.5V", "0.2V"};  // 縦軸表示文字（\0含んだ文字数が必要）
...
const char hRangeName[8][6] PROGMEM = {" 50ms", " 20ms", " 10ms",
                                       "  5ms", "  2ms", "  1ms",
                                       "500us", "200us"};  // 横軸(48バイト）
const char* const hstring_table[] PROGMEM = {
    hRangeName[0], hRangeName[1], hRangeName[2], hRangeName[3],
    hRangeName[4], hRangeName[5], hRangeName[6], hRangeName[7]};
...
volatile int vRange;  // 垂直レンジ  0:A50V, 1:A 5V, 2:50V, 3:20V, 4:10V, 5:5V,
                      // 6:2V, 7:1V, 8:0.5V
volatile int hRange;  // 水平レンジ　0:50m, 1:20m, 2:10m, 3:5m, 4;2m, 5:1m,
                      // 6:500u, 7;200u
volatile int trigD;  // トリガ方向（極性）0:ポジ、1:ネガ
volatile int
    scopeP;  // 操作スコープ位置 0:垂直レンジ, 1:水平レンジ, 2:トリガ方向
volatile boolean hold = false;  // ホールド
volatile boolean paraChanged =
    false;  // 割込みでパラメーターが変更された時に true
...
void setConditions() {  // 測定条件設定
    ...
    strcpy_P(chrBuff,
             (char*)pgm_read_word(&(hstring_table[hRange])));  // 横軸レンジ名を
    hScale = chrBuff;  // hScaleに設定

    // 縦軸レンジ設定
    strcpy_P(chrBuff,
             (char*)pgm_read_word(&(vstring_table[vRange])));  // 縦軸レンジ名を
...
void readWave() {             // 波形をメモリーに記録
    ...
    switch (hRange) {  // レンジに応じ記録速度を変更
        ...
        case 0: {  // 50msレンジ
            timeExec =
                400 +
                50;  // 概算実行時間(ms) EEPROM保存までのカウントダウンに使用
        ...
        case 4: {  // 2 msレンジ
            timeExec =
                ...
                50;  // 概算実行時間(ms) EEPROM保存までのカウントダウンに使用
            ADCSRA = ADCSRA & 0xf8;  // 下3ビットをクリア
            ADCSRA = ADCSRA | 0x06;  // 分周比64 (0x1=2, 0x2=4, 0x3=8, 0x4=16,
        ...
        case 5: {  // 1 msレンジ
            timeExec =
                ...
                50;  // 概算実行時間(ms) EEPROM保存までのカウントダウンに使用
            ADCSRA = ADCSRA & 0xf8;  // 下3ビットをクリア
            ADCSRA = ADCSRA | 0x05;  // 分周比16 (0x1=2, 0x2=4, 0x3=8, 0x4=16,
                                     // 0x5=32, 0x6=64, 0x7=128)
            for (int i = 0; i < REC_LENGTH; i++) {  // 200データー
        ...
        case 6: {  // 500usレンジ
            timeExec =
                ...
                50;  // 概算実行時間(ms) EEPROM保存までのカウントダウンに使用
            ADCSRA = ADCSRA & 0xf8;  // 下3ビットをクリア
            ADCSRA = ADCSRA | 0x04;  // 分周比16(0x1=2, 0x2=4, 0x3=8, 0x4=16,
                                     // 0x5=32, 0x6=64, 0x7=128)
            for (int i = 0; i < REC_LENGTH; i++) {  // 200データー
        ...
        case 7: {  // 200usレンジ
            timeExec =
                ...
                50;  // 概算実行時間(ms) EEPROM保存までのカウントダウンに使用
            ADCSRA = ADCSRA & 0xf8;  // 下3ビットをクリア
            ADCSRA = ADCSRA | 0x02;  // 分周比:4(0x1=2, 0x2=4, 0x3=8, 0x4=16,
                                     // 0x5=32, 0x6=64, 0x7=128)
...
void dataAnalize() {  // 作図のための各種情報を設定
    ...
    if (vRange <= 1) {  // Autoレンジなら（レンジ番号が1かそれ以下）
        ...
        if (att10x == 1) {  // アッテネータ有
            rangeMaxDisp =
                100 *
                (rangeMax *
                 lsb50V);  // 表示範囲はデーターで決める。つまり上限はADCのフルスケールまで
            rangeMinDisp =
                100 *
                (rangeMin * lsb50V);  // 下限は測定結果によるが、最低でもゼロ
        } else {                      // アッテネータ無し
    ...
    // トリガ位置を探す
    for (trigP = ((REC_LENGTH / 2) - 51); trigP < ((REC_LENGTH / 2) + 50);
    ...
    trigSync = true;
    if (trigP >=
        ((REC_LENGTH / 2) + 50)) {  // トリガが見つからなかったら中央にしておく
        trigP = (REC_LENGTH / 2);
...
void pin2IRQ() {  // Pin2(int0)割込みの処理
    ...
    if ((x & 0x07) !=
        0x07) {  // 下位3ビットが全部Highで無ければ（どれかが押されていたら）
