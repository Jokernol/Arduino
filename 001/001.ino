...
void dispVscale() {  // 縦軸目盛り表示
    display.fillRect(0, 9, 24, 55,
                     BLACK);  // 前の表示をまとめて消す（24x55ドット）(460us)
...
void dispNewData() {  // 最新データーの値を画面の右上に表示
    display.fillRect(104, 0, 24, 8,
                     BLACK);  // 前の表示値を消す(24x8ドット)(320us)
...
void plotData() {  // 配列の値に基づきデーターをプロット
    long yPoint;
    display.fillRect(27, 10, 100, 53,
                     BLACK);  // グラフ表示領域をクリア(100x53ドット)(1.9ms)
    for (int i = 0; i <= 98; i++) {
